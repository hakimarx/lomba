            ...
            if($res===false){
                ...
                if(strpos($err, "doesn't exist") !== false){
                    ...
                    if(preg_match("/`?([a-zA-Z0-9_]+)\.`?([a-zA-Z0-9_]+)`? doesn't exist/", $err, $m)){
                        ...
                        $full = $m2[1];
                        $parts = explode('.', $full);
                        if(count($parts)==2){ $dbname=$parts[0]; $table=$parts[1]; } else { $table=$parts[0]; }
                    } else {
                        $table=null;
                    }
                        ...
                        if($sqlFile && file_exists($sqlFile) && is_readable($sqlFile)){
                            ...
                            if($sqlContents!==false){
                                ...
                                if(mysqli_multi_query($koneksi, $sqlContents)){
                                    // flush results
                                    do { if($r = mysqli_store_result($koneksi)) mysqli_free_result($r); } while(mysqli_more_results($koneksi) && mysqli_next_result($koneksi));
